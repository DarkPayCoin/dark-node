...
parameter_types! {
	pub const MinStorefrontOwners: u16 = 1;
	pub const MaxStorefrontOwners: u16 = 1000;
...
impl Trait for Test {
  type Event = ();
  type MinStorefrontOwners = MinStorefrontOwners;
  type MaxStorefrontOwners = MaxStorefrontOwners;
...
fn _create_default_storefront_owners() -> DispatchResult {
  _create_storefront_owners(None, None, None, None)
}

fn _create_storefront_owners(
  origin: Option<Origin>,
  storefront_id: Option<StorefrontId>,
  owners: Option<Vec<AccountId>>,
  threshold: Option<u16>,
) -> DispatchResult {
  MultiOwnership::create_storefront_owners(
    origin.unwrap_or_else(|| Origin::signed(ACCOUNT1)),
    storefront_id.unwrap_or(1),
...
#[test]
fn create_storefront_owners_should_work() {
  new_test_ext().execute_with(|| {
    assert_ok!(_create_default_storefront_owners());

    // Check storages
    let mut check: Vec<u64> = MultiOwnership::storefront_ids_owned_by_account_id(ACCOUNT1).iter().cloned().collect();
    assert_eq!(check, vec![1]);

    check = MultiOwnership::storefront_ids_owned_by_account_id(ACCOUNT2).iter().cloned().collect();
    assert_eq!(check, vec![1]);

    // Check whether data is stored correctly
    let storefront_owners = MultiOwnership::storefront_owners_by_storefront_id(1).unwrap();
    assert_eq!(storefront_owners.owners, vec![ACCOUNT1, ACCOUNT2]);
    assert_eq!(storefront_owners.storefront_id, 1);
    assert_eq!(storefront_owners.threshold, 2);
    assert_eq!(storefront_owners.changes_count, 0);
  });
}
...
fn propose_change_should_work() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(MultiOwnership::pending_change_id_by_storefront_id(1), Some(1));
...
fn propose_change_should_fail_no_owners_left() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(_propose_change(
      ...
      Some(None),
      None
     ), Error::<Test>::NoStorefrontOwnersLeft);
  });
}
...
#[test]
fn propose_change_should_fail_not_a_storefront_owner() {
  new_test_ext().execute_with(|| {
    assert_ok!(_create_default_storefront_owners());
    assert_noop!(_propose_change(
      ...
      Some(Some(2)),
      None
     ), Error::<Test>::NotAStorefrontOwner);
  });
}
...
fn confirm_change_should_work_owner_added() {
  new_test_ext().execute_with(|| {
    ...
    // Check storages
    assert_eq!(MultiOwnership::pending_change_id_by_storefront_id(1), None);
    assert_eq!(MultiOwnership::executed_change_ids_by_storefront_id(1), vec![1]);
    assert_eq!(MultiOwnership::next_change_id(), 2);
    ...
    let storefront_owners = MultiOwnership::storefront_owners_by_storefront_id(1).unwrap();
    assert_eq!(storefront_owners.owners, vec![ACCOUNT1, ACCOUNT2, ACCOUNT3]);
    assert_eq!(storefront_owners.threshold, 3);
  });
}
...
fn confirm_change_should_work_owner_removed() {
  new_test_ext().execute_with(|| {
    ...
    // Check whether updates applied
    let storefront_owners = MultiOwnership::storefront_owners_by_storefront_id(1).unwrap();
    assert_eq!(storefront_owners.owners, vec![ACCOUNT1]);
    assert_eq!(storefront_owners.threshold, 1);
  });
}

#[test]
fn confirm_change_should_fail_not_related_to_storefront_owners() {
  new_test_ext().execute_with(|| {
    assert_ok!(_create_default_storefront_owners());
    assert_ok!(_propose_default_change());
    assert_ok!(_create_storefront_owners(
      Some(Origin::signed(ACCOUNT3)),
      Some(2),
      Some(vec![ACCOUNT3]),
      Some(1)
    ...
    assert_noop!(_confirm_change(
      ...
      Some(1),
      Some(2)
    ), Error::<Test>::ChangeNotRelatedToStorefront);
  });
}
...
#[test]
fn confirm_change_should_fail_not_a_storefront_owner() {
  new_test_ext().execute_with(|| {
    assert_ok!(_create_default_storefront_owners());
    assert_ok!(_propose_default_change());
    assert_noop!(_confirm_change(
...
#[test]
fn cancel_proposal_should_fail_not_related_to_storefront_owners() {
  new_test_ext().execute_with(|| {
    assert_ok!(_create_default_storefront_owners());
    assert_ok!(_propose_default_change());
    assert_ok!(_create_storefront_owners(
...
#[test]
fn cancel_proposal_should_fail_not_a_storefront_owner() {
  new_test_ext().execute_with(|| {
    assert_ok!(_create_default_storefront_owners());
    assert_ok!(_propose_default_change());
    assert_noop!(_cancel_change(
