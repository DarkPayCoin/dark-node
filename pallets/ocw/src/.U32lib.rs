...
pub const KEY_TYPE: KeyTypeId = KeyTypeId(*b"dark");
...
decl_module! {
    ...
    fn offchain_worker(block_number: T::BlockNumber) {
		...
		if start_type == T::BlockNumber::from(1u32) {
			...
			// running natively.
			debug::native::info!("▼ DARK oracle offchain workers | # {} ",block_number);
...
impl<T: Trait> Module<T> {
    ...
    fn choose_transaction_type(block_number: T::BlockNumber) -> TransactionType {
        ...
        let val = StorageValueRef::persistent(b"dark_ocw::last_send");
    ...
    fn fetch_price() -> Result<u32, http::Error> {
        ...
        let request = http::Request::get(
            "https://min-api.cryptocompare.com/data/price?fsym=D4RK&tsyms=USD"
        );
        ...
        let price = match Self::parse_price_cryptocompare(body_str) {
            Some(price) => Ok(price),
            None => {
        ...
        let readable_price = (price as f64) * 0.00001 ;
