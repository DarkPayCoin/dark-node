...
pub struct Order<T: Trait> {
    ...
    pub buyer_escrow: u32,
    pub seller_escrow: u32,
...
decl_module! {
  pub struct Module<T: Trait> for enum Call where origin: T::Origin {
   ...
   // const TestAmount: BalanceOf<T> = 666 * DOLLARS;
    ...
    pub fn create_order(
      ...
      <T as pallet_utils::Trait>::Currency::reserve(buyer,buyer_escrow_total)
      .map_err(|_| "locker can't afford to lock the amount requested")?;

 let now = <system::Module<T>>::block_number();

 Self::deposit_event(RawEvent::LockFunds(buyer.clone(), new_order_id));
